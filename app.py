...
SCHEDULE_PREFERENCE_FILE = Path("configs") / "crawl_schedule.json"
WEEKDAY_OPTIONS = [
    "Monday",
    "Tuesday",
    "Wednesday",
...
def load_schedule_preferences() -> dict:
    """Load the last saved schedule preference if it exists."""
    if not SCHEDULE_PREFERENCE_FILE.exists():
        return {}

    try:
        with open(SCHEDULE_PREFERENCE_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
...
def save_schedule_preferences(preference: dict) -> str:
    """Persist schedule preference so the user can reuse it later."""
    SCHEDULE_PREFERENCE_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(SCHEDULE_PREFERENCE_FILE, "w", encoding="utf-8") as f:
        json.dump(preference, f, ensure_ascii=False, indent=2)
    return str(SCHEDULE_PREFERENCE_FILE)


def clear_schedule_preferences() -> bool:
    """Remove the saved schedule preference file."""
    try:
        if SCHEDULE_PREFERENCE_FILE.exists():
            SCHEDULE_PREFERENCE_FILE.unlink()
        return True
    except Exception:
        return False


def calculate_next_schedule_run(schedule_frequency: str, schedule_time, schedule_weekday: str = "Monday"):
    """Calculate the next run time for a daily or weekly schedule."""
    now = datetime.now()
    target_time = datetime.combine(now.date(), schedule_time)

    if schedule_frequency == "Weekly":
        weekday_index = WEEKDAY_OPTIONS.index(schedule_weekday) if schedule_weekday in WEEKDAY_OPTIONS else 0
        days_ahead = (weekday_index - now.weekday()) % 7
        if days_ahead == 0 and target_time <= now:
            days_ahead = 7
        return datetime.combine(now.date() + timedelta(days=days_ahead), schedule_time)
...
def render_schedule_command_preview(
    spider_key: str,
    params: dict,
    output_format: str,
    llm_config: dict,
    preview_url: str,
    auto_name_by_domain: bool,
    organize_by_domain: bool,
    new_file_always: bool,
) -> str:
    """Build a sanitized command preview for external schedulers."""
    preview_output = get_output_path(
        url=preview_url,
    ...
    schedule_llm_config = {k: v for k, v in llm_config.items() if k != "api_key"}
    command = build_command(
        spider_key,
        params,
        preview_output,
        output_format,
        schedule_llm_config if schedule_llm_config.get("enabled") else None,
    )
    return subprocess.list2cmdline(command)


def build_schedule_params(spider_key: str, target_url: str, current_params: dict | None = None) -> dict:
    """Build a minimal parameter set for a scheduled spider run."""
    spider_params = SPIDERS.get(spider_key, {}).get("params", {})
    schedule_params = {key: config.get("default", "") for key, config in spider_params.items()}

    if current_params:
        for key in schedule_params:
            if key in current_params and current_params.get(key) not in (None, ""):
                schedule_params[key] = current_params[key]

    target_url = str(target_url).strip()
    if spider_key == "drug":
        schedule_params["start_url"] = target_url
    else:
        schedule_params["urls"] = target_url
...
- `drug_page` - Force URL as direct drug page (will extract content)
    ...
    st.subheader("⏰ Schedule Preference")
    st.caption("Pick a spider first, then save the time and repeat settings for an unattended run.")

    schedule_spider_options = list(SPIDERS.keys())
    current_schedule_spider = st.session_state.get("schedule_spider_key", spider_key)
    if current_schedule_spider not in schedule_spider_options:
        current_schedule_spider = spider_key if spider_key in schedule_spider_options else schedule_spider_options[0]

    st.session_state.schedule_spider_key = st.selectbox(
        "Spider to schedule",
        options=schedule_spider_options,
        index=schedule_spider_options.index(current_schedule_spider),
        format_func=lambda key: SPIDERS[key]["name"],
        key="schedule_spider_key",
        help="Choose which spider this schedule profile should run.",
    )

    schedule_spider_key = st.session_state.schedule_spider_key
    schedule_spider_config = SPIDERS[schedule_spider_key]

    if schedule_spider_key != spider_key:
        st.info(f"The schedule card is now set to {schedule_spider_config['name']}. Switch to that spider tab to adjust its crawl fields if needed.")

    saved_schedule = load_schedule_preferences()
    if saved_schedule.get("spider") == schedule_spider_key and st.session_state.get("schedule_pref_loaded_for") != schedule_spider_key:
        st.session_state.schedule_enabled = bool(saved_schedule.get("enabled", False))
        st.session_state.schedule_frequency = saved_schedule.get("frequency", "Daily")
        st.session_state.schedule_weekday = saved_schedule.get("weekday", "Monday")
        st.session_state.schedule_url = saved_schedule.get("url", "")
        saved_time = str(saved_schedule.get("time", "09:00"))
        try:
            hours, minutes = [int(part) for part in saved_time.split(":")[:2]]
            st.session_state.schedule_time = datetime.now().replace(hour=hours, minute=minutes, second=0, microsecond=0).time()
        except Exception:
            st.session_state.schedule_time = datetime.now().replace(hour=9, minute=0, second=0, microsecond=0).time()
        st.session_state.schedule_pref_loaded_for = schedule_spider_key

    if "schedule_enabled" not in st.session_state:
        st.session_state.schedule_enabled = False
    if "schedule_frequency" not in st.session_state:
        st.session_state.schedule_frequency = "Daily"
    if "schedule_weekday" not in st.session_state:
        st.session_state.schedule_weekday = "Monday"
    if "schedule_time" not in st.session_state:
        st.session_state.schedule_time = datetime.now().replace(hour=9, minute=0, second=0, microsecond=0).time()
    if "schedule_url" not in st.session_state:
        st.session_state.schedule_url = preview_url or ""

    if schedule_spider_key == spider_key:
        schedule_default_url = preview_url
    else:
        schedule_default_url = st.session_state.schedule_url or preview_url

    schedule_url_label = "Target URL"
    if schedule_spider_key == "drug":
        schedule_url_label = "Start URL"
    elif schedule_spider_key == "geometric":
        schedule_url_label = "URLs (comma-separated)"

    st.session_state.schedule_url = st.text_input(
        schedule_url_label,
        value=schedule_default_url,
        key="schedule_url",
        help="This URL will be used when the scheduled run starts.",
    )

    if st.session_state.schedule_url:
        schedule_time_value = st.time_input(
            "Run time",
            value=st.session_state.schedule_time,
            key="schedule_time",
            help="Choose the local time of day when the crawler should run.",
        )
        st.session_state.schedule_time = schedule_time_value

        schedule_frequency = st.radio(
            "Repeat",
            options=["Daily", "Weekly"],
            index=0 if st.session_state.schedule_frequency == "Daily" else 1,
            horizontal=True,
            key="schedule_frequency",
        )

        if schedule_frequency == "Weekly":
            st.session_state.schedule_weekday = st.selectbox(
                "Day of week",
                options=WEEKDAY_OPTIONS,
                index=WEEKDAY_OPTIONS.index(st.session_state.schedule_weekday) if st.session_state.schedule_weekday in WEEKDAY_OPTIONS else 0,
                key="schedule_weekday",
            )
        else:
            st.session_state.schedule_weekday = "Monday"

        st.session_state.schedule_enabled = st.checkbox(
            "Enable this schedule preference",
            value=st.session_state.schedule_enabled,
            key="schedule_enabled",
            help="When enabled, the saved schedule profile can be reused for unattended runs.",
        )

        next_run = calculate_next_schedule_run(
            st.session_state.schedule_frequency,
            st.session_state.schedule_time,
            st.session_state.schedule_weekday,
        )
        st.info(f"Next run preview: {next_run.strftime('%Y-%m-%d %H:%M')}")

        schedule_params = build_schedule_params(
            schedule_spider_key,
            st.session_state.schedule_url,
            params if schedule_spider_key == spider_key else None,
        )

        schedule_command_preview = render_schedule_command_preview(
            spider_key=schedule_spider_key,
            params=schedule_params,
            output_format=output_format,
            llm_config=llm_config,
            preview_url=st.session_state.schedule_url,
            auto_name_by_domain=auto_name_by_domain,
            organize_by_domain=organize_by_domain,
            new_file_always=new_file_always,
        )
        st.code(schedule_command_preview, language="bash")

        schedule_col1, schedule_col2 = st.columns(2)
        with schedule_col1:
            if st.button("💾 Save Schedule Preference", key="save_schedule_preference_btn"):
                schedule_payload = {
                    "spider": schedule_spider_key,
                    "enabled": bool(st.session_state.schedule_enabled),
                    "frequency": st.session_state.schedule_frequency,
                    "weekday": st.session_state.schedule_weekday if st.session_state.schedule_frequency == "Weekly" else None,
                    "time": st.session_state.schedule_time.strftime("%H:%M"),
                    "url": st.session_state.schedule_url,
                    "output_format": output_format,
                    "params": schedule_params,
                    "parallel_config": st.session_state.get("parallel_config", {}),
                    "jobdir_mode": st.session_state.get("jobdir_mode", "Auto (recommended)"),
                    "custom_jobdir": st.session_state.get("custom_jobdir", ""),
                    "auto_name_by_domain": auto_name_by_domain,
                    "organize_by_domain": organize_by_domain,
                    "new_file_always": new_file_always,
                    "llm": {
                ...
                saved_path = save_schedule_preferences(schedule_payload)
                st.session_state.schedule_pref_loaded_for = schedule_spider_key
                st.success(f"Schedule preference saved to {saved_path}")
        with schedule_col2:
            if st.button("🧹 Clear Saved Schedule", key="clear_schedule_preference_btn"):
                if clear_schedule_preferences():
                    st.session_state.schedule_pref_loaded_for = None
                    st.session_state.schedule_enabled = False
                    st.session_state.schedule_frequency = "Daily"
                    st.session_state.schedule_weekday = "Monday"
                    st.session_state.schedule_time = datetime.now().replace(hour=9, minute=0, second=0, microsecond=0).time()
                    st.session_state.schedule_url = preview_url or ""
                    st.success("Saved schedule preference cleared")
                else:
                    st.error("Could not clear the saved schedule preference")
    else:
        st.warning("Enter a URL first so the schedule command can be generated.")
